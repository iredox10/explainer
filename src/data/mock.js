export const AUTHORS = [
    {
        slug: "chioma-okafor",
        name: "Chioma Okafor",
        role: "Senior Economics Reporter",
        bio: "Chioma Okafor covers the intersection of technology, policy, and economics in West Africa. She was previously a fellow at the Nieman Foundation.",
        email: "chioma@vox.africa",
        social: {
            twitter: "@chioma_o",
            linkedin: "linkedin.com/in/chioma"
        }
    },
    {
        slug: "chioma-okereke",
        name: "Chioma Okereke",
        role: "Senior Tech Correspondent",
        bio: "Chioma Okereke breaks stories on startups, venture capital, and the digital infrastructure of the continent. Her work has appeared in Wired and TechCrunch.",
        email: "c.okereke@vox.africa",
        social: {
            twitter: "@cokereke_tech",
            website: "chiomaokereke.com"
        }
    },
    {
        slug: "tunde-alabi",
        name: "Dr. Tunde Alabi",
        role: "Network Architect & Contributor",
        bio: "Tunde Alabi is a network engineer with 15 years of experience building fiber optic networks. He writes about the physical side of the internet.",
        email: "guest@vox.africa",
        social: {
            twitter: "@tunde_networks"
        }
    },
    {
        slug: "yara-shahidi",
        name: "Yara Shahidi",
        role: "Politics Contributor",
        bio: "Yara Shahidi is a political analyst focusing on youth movements and electoral reform.",
        social: {
            twitter: "@yara_s"
        }
    },
    {
        slug: "carlos-m",
        name: "Carlos M.",
        role: "Global Affairs",
        bio: "Carlos covers international relations and migration patterns.",
        social: {
            twitter: "@carlos_global"
        }
    },
    {
        slug: "sarah-jenkins",
        name: "Sarah Jenkins",
        role: "Climate Reporter",
        bio: "Sarah Jenkins reports on the human impact of climate change in the global south.",
        social: {
            twitter: "@sarah_climate"
        }
    },
    {
        slug: "vox-staff",
        name: "Vox Staff",
        role: "Editorial Team",
        bio: "Reports and analysis from the Explainer newsroom.",
        social: {}
    }
];

export const VOX_HOME_DATA = {
    hero: {
        headline: "The Giant Wakes: How Nigeria is Reshaping the Global Economy",
        subhead: "From tech unicorns to cultural exports, Lagos is becoming the center of African innovation.",
        author: "By Chioma Okafor",
        authorSlug: "chioma-okafor",
        imageUrl: "https://images.unsplash.com/photo-1569163139599-0f4517e36f51?w=1600&q=80",
        category: "Super Feature",
        slug: "giant-wakes"
    },
    latest: [
        {
            title: "The hidden fiber optic cables connecting Lagos to the world",
            category: "Technology",
            imageUrl: "https://images.unsplash.com/photo-1544197150-b99a580bbcbf?w=800&q=80",
            readTime: "8 min",
            slug: "fiber-optic-lagos",
            seriesSlug: "africa-tech-boom",
            author: "Chioma Okereke",
            authorSlug: "chioma-okereke"
        },
        {
            title: "Why your rent is so high (and why it's not going down)",
            category: "Economy",
            imageUrl: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=800&q=80",
            readTime: "8 min",
            slug: "fiber-optic-lagos",
            author: "Chioma Okafor",
            authorSlug: "chioma-okafor"
        },
        {
            title: "The hidden cost of 'fast fashion' isn't just environmental",
            category: "Culture",
            imageUrl: "https://images.unsplash.com/photo-1523381210434-271e8be1f52b?w=800&q=80",
            readTime: "6 min",
            slug: "fiber-optic-lagos",
            author: "Vox Staff",
            authorSlug: "vox-staff"
        },
        {
            title: "How AI is quietly rewriting the internet",
            category: "Technology",
            imageUrl: "https://images.unsplash.com/photo-1620712943543-bcc4688e7485?w=800&q=80",
            readTime: "12 min",
            slug: "fiber-optic-lagos",
            author: "Chioma Okereke",
            authorSlug: "chioma-okereke"
        }
    ],
    videos: [
        {
            title: "How maps distort your view of the world",
            seriesName: "Atlas",
            duration: "10:45",
            thumbUrl: "https://images.unsplash.com/photo-1524661135-423995f22d0b?w=800&q=80"
        },
        {
            title: "Why American public transit is so bad",
            seriesName: "Urbanity",
            duration: "14:20",
            thumbUrl: "https://images.unsplash.com/photo-1570125909232-eb263c188f7e?w=800&q=80"
        },
        {
            title: "The intense engineering of F1 cars",
            seriesName: "Speed",
            duration: "12:15",
            thumbUrl: "https://images.unsplash.com/photo-1532906619279-a75a0fdd75ad?w=800&q=80"
        }
    ],
    politics: [
        {
            title: "The new geopolitical map of Africa",
            imageUrl: "https://images.unsplash.com/photo-1529101091760-61df6be16102?w=800&q=80",
            author: "Yara Shahidi",
            authorSlug: "yara-shahidi"
        },
        {
            title: "Why voting laws are changing everywhere",
            imageUrl: "https://images.unsplash.com/photo-1540910419868-474947ce531c?w=800&q=80",
            author: "Carlos M.",
            authorSlug: "carlos-m"
        },
        {
            title: "The climate refugees no one talks about",
            imageUrl: "https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=800&q=80",
            author: "Sarah Jenkins",
            authorSlug: "sarah-jenkins"
        }
    ],
    culture: [
        {
            title: "Afrobeats is taking over the world. Here's why.",
            imageUrl: "https://images.unsplash.com/photo-1493225255756-d9584f8606e9?w=800&q=80",
            category: "Music",
            author: "Vox Staff",
            authorSlug: "vox-staff"
        },
        {
            title: "The chaotic joy of Lagos street fashion",
            imageUrl: "https://images.unsplash.com/photo-1581338834647-b0fb40704e21?w=800&q=80",
            category: "Style",
            author: "Vox Staff",
            authorSlug: "vox-staff"
        },
        {
            title: "Nollywood's new golden age",
            imageUrl: "https://images.unsplash.com/photo-1489599849927-2ee91cede3ba?w=800&q=80",
            category: "Film",
            author: "Vox Staff",
            authorSlug: "vox-staff"
        },
        {
            title: "Why everyone is reading African sci-fi",
            imageUrl: "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=800&q=80",
            category: "Books",
            author: "Vox Staff",
            authorSlug: "vox-staff"
        }
    ],
    futurePerfect: [
        {
            title: "Can we cool the planet by blocking the sun?",
            subhead: "Solar geoengineering is risky, controversial, and might be our only hope.",
            imageUrl: "https://images.unsplash.com/photo-1532094349884-543bc11b234d?w=800&q=80",
            author: "Sarah Jenkins",
            authorSlug: "sarah-jenkins"
        },
        {
            title: "The end of meat is closer than you think",
            subhead: "Lab-grown steak is hitting shelves. Is it any good?",
            imageUrl: "https://images.unsplash.com/photo-1615937657715-bc7b4b7962c1?w=800&q=80",
            author: "Chioma Okereke",
            authorSlug: "chioma-okereke"
        },
        {
            title: "How AI discovers new drugs",
            subhead: "DeepMind's latest breakthrough could cure malaria.",
            imageUrl: "https://images.unsplash.com/photo-1532187863486-abf9dbad1b69?w=800&q=80",
            author: "Chioma Okereke",
            authorSlug: "chioma-okereke"
        }
    ],
    series: {
        "africa-tech-boom": {
            title: "The Africa Tech Boom",
            slug: "africa-tech-boom",
            description: "A comprehensive guide to the infrastructure, startups, and culture shaping the continent's digital future.",
            heroImage: "https://images.unsplash.com/photo-1518432031352-d6fc5c10da5a?q=80&w=2574",
            articles: [
                {
                    title: "The hidden fiber optic cables connecting Lagos to the world",
                    slug: "fiber-optic-lagos",
                    partNumber: 1
                },
                {
                    title: "The 50 most influential startups in Yaba",
                    slug: "fiber-optic-lagos", // reusing slug so it doesn't 404
                    partNumber: 2
                },
                {
                    title: "Afrobeats is taking over the world. Here's why.",
                    slug: "fiber-optic-lagos", // reusing slug
                    partNumber: 3
                }
            ]
        }
    }
};