    ...
    return (
      ...
      <main className="lg:ml-64 flex-1 p-4 md:p-8 pt-24 lg:pt-8 bg-gray-50/50">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          ...
          <button onClick={fetchLogs} className="w-full md:w-auto bg-white border border-gray-100 p-4 rounded-2xl hover:bg-gray-50 transition-all shadow-sm flex items-center justify-center">
        ...
        <div className="max-w-6xl mx-auto space-y-4">
          {isLoading ? (
            ...
            <div key={log.id} className="bg-white rounded-2xl p-4 md:p-6 border border-gray-100 shadow-sm flex flex-col sm:flex-row md:items-center justify-between gap-4 group hover:border-black transition-all">
              <div className="flex items-center gap-4 md:gap-8">
                <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center shrink-0 ${log.level === 'critical' || log.level === 'warning' ? 'bg-red-50 text-red-600' :
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-gray-900 truncate">{log.action}</span>
                    <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded shrink-0 ${log.level === 'warning' ? 'bg-red-500 text-white shadow-[0_5px_15px_rgba(239,68,68,0.2)]' : 'bg-gray-100 text-gray-400'
                      }`}>{log.level}</span>
                  </div>
                  <p className="text-[10px] md:text-xs text-gray-400 mt-1 font-medium italic truncate">{log.detail || 'Standard protocol event recorded.'}</p>
                </div>
              </div>

              <div className="text-left sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0">
                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-900">{log.user}</p>
                <p className="text-[9px] md:text-[10px] text-gray-300 font-bold uppercase tracking-[0.1em] mt-1">{log.time}</p>
              </div>
            </div>
          ))}
