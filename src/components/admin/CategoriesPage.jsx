...
export default function CategoriesPage() {
  ...
  const [editingCategory, setEditingCategory] = useState(null); // Can be a new category or an existing one
            ...
            <tbody className="divide-y divide-gray-100">
              {categories.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <code className="text-xs bg-gray-100 px-2 py-1 rounded border border-gray-200 text-gray-600">{cat.slug}</code>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded-full">{cat.count}</span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                        <button onClick={() => openEditModal(cat)} className="p-2 text-gray-400 hover:text-[#008751] hover:bg-gray-100 rounded-lg transition-colors">
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => handleModalChange('slug', e.target.value)}
                    required
                  />
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-sm font-mono text-gray-600">{editingCategory.color}</span>
                  </div>
                </div>
