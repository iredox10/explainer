...
export default function AuthorsPage() {
  ...
  const handleDelete = (id) => {
    if (confirm("Are you sure you want to remove this author profile?")) {
            ...
            <div key={author.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:border-[#008751] transition-colors group">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-1">
                   <button onClick={() => openEditModal(author)} className="p-2 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-md transition-colors">
                     <Edit2 className="w-4 h-4" />
                   </button>
                   <button onClick={() => handleDelete(author.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-md transition-colors">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in duration-200">
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                <h3 className="font-bold text-lg text-gray-900">{editingAuthor.id ? 'Edit Author Profile' : 'Add New Author'}</h3>
              ...
              <form onSubmit={handleSave} className="p-6 space-y-4">
                ...
                <div className="pt-4 flex justify-end gap-3">
                  ...
                  >
                    ...
                    {editingAuthor.id ? 'Save Changes' : 'Create Profile'}
                  </button>
                </div>
              </form>
            </div>
